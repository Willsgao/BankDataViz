<template>
  <div class="pdf-preview-container">
    ...
    <div style="background: red; color: white; padding: 10px;">
      <p>selectedPdf: {{ selectedPdf }}</p>
      <p>pdfUrl: {{ pdfUrl }}</p>
      <p>pdfFiles: {{ pdfFiles }}</p>
      <p>currentPdfIndex: {{ currentPdfIndex }}</p>
    </div>
...
// 修复1：添加缺失的getPdfUrl函数
const getPdfUrl = (pdfFile) => {
  if (!pdfFile) return ''
  const diskName = pdfFile.disk_name || pdfFile.filename
  return getBackendUrl(`/api/file/${diskName}`)
}

// 修复2：修正effectivePdf计算逻辑
const effectivePdf = computed(() => {
  // 优先使用selectedPdf（向后兼容）
  if (props.selectedPdf) {
    return props.selectedPdf
  }
  // 其次使用pdfFiles数组中的当前PDF
  if (props.pdfFiles && props.pdfFiles.length > 0) {
    const index = props.currentPdfIndex >= 0 ? props.currentPdfIndex : 0
    return props.pdfFiles[index]
  }
  return null
})

// 修复3：修正effectivePdfUrl计算逻辑
const effectivePdfUrl = computed(() => {
  return props.pdfUrl || getPdfUrl(effectivePdf.value)
})
...
watch(() => effectivePdfUrl.value, (newUrl) => {
  if (newUrl && usePdfJs.value) {
    loadPdf() // 只有启用PDF.js时才加载
  }
}, { immediate: true })
