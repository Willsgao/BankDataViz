...
class TableDataAligner:
    ...
    def extract_leaf_nodes(self, hierarchy_fields: List[Dict]) -> List[Dict]:
        ...
        for field in hierarchy_fields:
            ...
            if field_path:
                ...
                leaf_nodes.append({
                    'full_path': field_value,
                    'leaf_text': field_value,
                    'original_field': field
                })
            elif field_type:
                leaf_nodes.append({
                    'full_path': field_type,
                    'leaf_text': field_type,
                    'original_field': field
                })

            print(
                f"     字段详情: path='{field_path}', leaf_text='{leaf_nodes[-1]['leaf_text'] if leaf_nodes else ''}'")
    ...
    def _calculate_directional_similarity(self, directional_leaves: List[Dict], ocr_leaves: List[str],
                                          direction: str) -> float:
        """
    ...
    def _find_vertical_header_column(self, cell_map: Dict, max_row: int, max_col: int, data_start_row: int) -> int:
        """
        识别纵向表头列：找到包含文本内容最多的列（排除序号列）
        """
        # 统计每列的非空文本数量
        column_text_counts = {}
        ...
        for col in range(max_col + 1):
            ...
            for row in range(data_start_row, max_row + 1):
                if (row, col) in cell_map:
                    text = cell_map[(row, col)].strip()
                    if text and not text.isdigit():  # 排除纯数字
                        text_count += 1
            column_text_counts[col] = text_count

        # 找到文本数量最多的列（排除第0列，因为通常是序号）
        if column_text_counts:
            # 从第1列开始找（跳过序号列）
            candidate_cols = {col: count for col, count in column_text_counts.items() if col >= 1}
            if candidate_cols:
                vertical_header_col = max(candidate_cols, key=candidate_cols.get)
                print(f"   各列文本统计: {column_text_counts}")
                print(f"   选择第{vertical_header_col + 1}列作为纵向表头列")
                return vertical_header_col

        return 1  # 默认返回第1列（序号列之后）




    def _find_real_data_start_row(self, cell_map: Dict, max_row: int, max_col: int) -> int:
        """找到真正的数据起始行（第一个包含数字数据的行）"""
    ...
    def _is_header_row(self, cell_map: Dict, row: int, max_col: int, llm_horizontal: List[Dict]) -> bool:
        """检测指定行是否为表头行"""
        for col in range(max_col + 1):
            if (row, col) in cell_map:
