...
# ===== 2. 生成 PDF =====
def make_pdf(table_data, filename="英语作文评分表.pdf"):
    ...
    elements = [Paragraph("2025 年春季高二英语作文评分表", cn_style),
                Spacer(1, 0.5*cm)]

    rows_per_page = 20
    total_pages = (len(table_data) - 1) // rows_per_page + 1

    for page in range(total_pages):
        start = 1 + page * rows_per_page
        end = start + rows_per_page
        chunk = [table_data[0]] + table_data[start:end]

        t = Table(chunk, colWidths=[1*cm, 3*cm, 2*cm, 2*cm, 2*cm, 2*cm, 1.5*cm, 5*cm])
        ...
        elements.append(Paragraph(f"第 {page+1} 页 / 共 {total_pages} 页", cn_style))
