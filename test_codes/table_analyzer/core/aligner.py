...
class TableDataAligner:
    ...
    def _rebuild_table_with_llm_headers111(self, ws, table: Dict, start_row: int):
        """重构版：支持多级表头合并"""
        ocr_data = table.get('ocr_data', {})
    ...
    def _find_vertical_header_column_enhanced111(self, cell_map: Dict, row: int, max_col: int,
            ...
            if ' > ' in llm_full_path:
                ...
                if len(parts) == 2:
                    ...
                    if best_col is not None:
                        ...
                        print(
                            f"     ✅ 匹配列{best_col}: OCR='{ocr_level1.get(best_col, '')}' -> LLM='{llm_full_path}'")
                    else:
                        # 如果没有匹配到，尝试右端对齐放置
    ...
    def _process_single_level_headers111(self, ws, cell_map, max_row, max_col,
                                      llm_horizontal, llm_vertical, start_row):
        """
        单级表头处理（原逻辑的封装）
        """
        ...
        # 如果LLM表头少于OCR表头，从右端开始替换（保留第一列）
            ...
            for i in range(replace_count):
                ...
                if llm_index >= 0 and ocr_index >= 1:  # ocr_index >= 1 跳过第一列
                    ...
                    print(
                        f"     ✅ 右端替换列{ocr_index}: '{ocr_headers[ocr_index]}' -> '{final_headers[ocr_index]}'")
    ...
    def _rebuild_table_with_llm_headers111(self, ws, table: Dict, start_row: int):
        ...
        print(f"📊 OCR表格结构: {max_row + 1}行 x {max_col + 1}列，单元格总数: {len(cell_map)}")

        # 2. 简化：直接按行列重建表格，不使用复杂的表头逻辑
        return self._simple_rebuild_table(ws, cell_map, max_row, max_col, start_row)

    def _simple_rebuild_table(self, ws, cell_map: Dict, max_row: int, max_col: int, start_row: int):
        """简化版表格重建：直接将OCR数据写入Excel"""
        print(f"🔄 使用简化表格重建，共{max_row + 1}行 x {max_col + 1}列")
        ...
        for row in range(max_row + 1):
            for col in range(max_col + 1):
                if (row, col) in cell_map:
                    ...
                    if value:  # 只写入非空值
                        ws.cell(row=start_row + row, column=col + 1, value=value)

        # 设置合适的列宽
        for col in range(1, max_col + 2):
            ws.column_dimensions[get_column_letter(col)].width = 15

        return start_row + max_row + 1
    ...
    def _process_single_level_headers(self, ws, cell_map, max_row, max_col,
        ...
        # 如果LLM表头少于OCR表头，从右端开始替换（保留第一列）
            ...
            for i in range(replace_count):
                ...
                if llm_index >= 0 and ocr_index >= 1 and llm_index < len(llm_horizontal):  # ocr_index >= 1 跳过第一列
                    llm_header = llm_horizontal[llm_index]
                    final_headers[ocr_index] = llm_header.get('full_path', '')
        ...
        # OCR数据中，表头通常在行0，数据从行1开始
        data_start_row = 1
        print(f"   数据起始行固定为: {data_start_row}")
        ...
        for row in range(data_start_row, max_row + 1):
            ...
            row_empty = True
            for col in range(max_col + 1):
                if (row, col) in cell_map and cell_map[(row, col)]:
                    row_empty = False
                    break
            ...
            # 处理数据列（跳过纵向表头列）
            for col in range(max_col + 1):
                if vertical_header_col is not None and col == vertical_header_col:
                    continue  # 跳过已经处理的纵向表头列
    ...
    def _rebuild_table_with_llm_headers(self, ws, table: Dict, start_row: int):
        ...
        print(f"🔄 处理表格: {table.get('table_title', '未知')}")
        ...
        for cell in body_cells:
            row_start = cell.get('row_start', 0)
            col_start = cell.get('col_start', 0)
            all_rows.add(row_start)
            all_cols.add(col_start)
        ...
        print(f"   表格大小: {max_row + 1}行 x {max_col + 1}列")

        # 2. 构建正确的cell_map（避免重复填充）
        cell_map = {}
        # 按单元格出现顺序处理，避免覆盖
        for cell in sorted(body_cells, key=lambda x: (x.get('row_start', 0), x.get('col_start', 0))):
            ...
            if (row, col) in cell_map and cell_map[(row, col)]:
                # 如果已存在非空内容，跳过（避免重复）
                print(f"   ⚠️ 单元格({row},{col})已有内容: '{cell_map[(row, col)]}'，跳过新内容: '{words}'")
                continue
        ...
        for col in range(max_col + 1):
            ...
            for row in range(max_row + 1):
                if (row, col) in cell_map:
                    column_contents[col].append(cell_map[(row, col)])

        print(f"   列内容统计:")
        for col in sorted(column_contents.keys()):
            non_empty = [c for c in column_contents[col] if c]
            print(f"     列{col}: {len(non_empty)}个非空值")
        ...
        columns_to_write = []
        for col in range(max_col + 1):
            non_empty_count = sum(1 for row in range(max_row + 1)
                                  if (row, col) in cell_map and cell_map[(row, col)])
            if non_empty_count > 0:
                columns_to_write.append(col)

        print(f"   实际写入列: {columns_to_write}")

        for row in range(max_row + 1):
            row_has_data = False
            for col_idx, col in enumerate(columns_to_write):
                if (row, col) in cell_map:
                    value = cell_map[(row, col)]
                    if value:  # 只写入非空值
        ...
        # 5. 设置列宽
        for col_idx in range(len(columns_to_write)):
            ws.column_dimensions[get_column_letter(col_idx + 1)].width = 20

        print(f"✅ 表格写入完成，共{current_row - start_row}行")
        return current_row