...
from backend.services.llm.utils import validate_required_params, convert_to_excel_url
from backend.services.llm.task_management_service import _send_websocket_notification, logger
from backend.services.llm.single_table_service import _process_single_non_financial_table
...
def _fallback_to_batch_format(output_file, processing_results, table_type):
    """回退到批量处理格式保存"""
    try:
        from backend.services.llm.utils import export_processing_results_to_excel
        ...
        if not excel_saved:
            # 如果详细导出失败，至少确保基础文件存在
            from backend.services.llm.utils import ensure_excel_file_exists
...
async def _analyze_processed_data(self, task_id: str, processing_results: Dict, output_file: str):
    """分析处理后的表格数据"""
    try:
        from backend.services.table_analysis_service import TableAnalysisService
