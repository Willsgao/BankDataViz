...
class TableOCRService:
    ...
    def __init__(self, api_key: str = None, secret_key: str = None, timeout: int = None, max_retries: int = None):
        ...
        Args:
            api_key: 百度云API Key，如果为None则从配置文件读取
            secret_key: 百度云Secret Key，如果为None则从配置文件读取
            timeout: 请求超时时间，如果为None则从配置文件读取
    ...
    def recognize_table_from_file(self, file_path: str, urlencoded: bool = True) -> Dict[str, Any]:
        ...
        return self.recognize_table_from_base64(image_base64)
    ...
    def save_result_to_json(self, result: Dict[str, Any], save_path: str) -> None:
        """
        将识别结果保存为 JSON 文件
        :param result: 识别返回的 dict
        :param save_path: 目标文件路径（含文件名）
        """
...
if __name__ == '__main__':
    ...
    page_file = r"E:\Datas\base_pros\DocuVista\test_codes/银行3.png"
