...
async def _process_single_image(data):
    ...
    try:
        ...
        # # ⭐⭐⭐ 首先检查Excel缓存是否存在 ⭐⭐⭐
        # cache_result = _check_excel_cache_with_validation(
        #     image_path,
        #     output_path,
        #     sheet_name,
        #     "financial"
...
def recognize_table_internal1111():
    """识别表格并保存到指定路径 - 先检查Excel是否存在"""
    try:
        data = request.get_json()
        print(f"收到识别请求: {data}")
...
async def _process_single_image122(data):
    """处理单张图片的异步函数 - 添加Excel存在性检查"""
    try:
        # 验证必要参数
        is_valid, error_msg = validate_required_params(
...
async def _process_non_financial_table1111(data):
    """处理普通表格的异步函数 - 添加Excel存在性检查"""
    try:
        # 验证必要参数
        is_valid, error_msg = validate_required_params(
