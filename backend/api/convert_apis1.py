...
def _ensure_table_processing_db():
    ...
    try:
        init_table_processing_db()  # 调用原来的初始化函数
...
def api_process_tables(pdf_folder: str):
    ...
    try:
        ...
        thread = threading.Thread(
            target=process_tables_async,
            args=(job_id, pdf_folder, valid_images, bank_name),
            daemon=True
        )
        thread.start()
...
def api_table_history():
    ...
    try:
        # 从数据库加载记录
        records = load_table_processing_records(limit=50)
...
def api_table_task_detail(job_id: str):
    ...
    try:
        # 先从内存查找
        if job_id in TABLE_PROCESSING_JOBS:
            task_detail = TABLE_PROCESSING_JOBS[job_id].copy()
        else:
            # 从数据库查找
            with closing(sqlite3.connect(DATABASE_PATH)) as conn:
                conn.row_factory = sqlite3.Row
