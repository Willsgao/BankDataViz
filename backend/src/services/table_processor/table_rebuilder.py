...
class TableReconstructor:
    ...
    def process_all_tables_to_memory00(self, ocr_result, llm_result, image_path=None, bank_name="未知银行"):
        """
        处理表格并返回内存中的数据（不保存文件）
    ...
    def process_all_tables_to_memory(self, ocr_result, llm_result, image_path=None, bank_name="未知银行"):
        ...
        for group_idx, group in enumerate(merge_groups):
            ...
            for table_idx in group:
                ...
                print(f"处理表格 {table_idx + 1}/{len(llm_tables)} (合并组)")
                print(f"表格名称: '{full_table_name}'")
                print(f"表格元数据: 币种={table_metadata['default_currency']}, "
                      f"报告期={table_metadata['default_report_period']}, "
                      f"单位={table_metadata['default_unit']}")
    ...
    def process_all_tables00(self, ocr_result, llm_result, output_file, final_output_file=None, image_path=None,
                           bank_name=""):
        """
        处理所有表格并保存到Excel文件
    ...
    def process_all_tables11(self, ocr_result, llm_result, output_file, final_output_file=None, image_path=None,
                           bank_name=""):
        """
        处理所有表格并保存到Excel文件
