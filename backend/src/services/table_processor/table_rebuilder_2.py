...
class TableReconstructor:
    ...
    def _validate_table_data101(self, table):
        """
        验证表格数据，标记可疑的行和列
    ...
    def _analyze_column_features101(self, table):
        """分析每列的数据特征"""
        if not table:
            return {}
    ...
    def _check_isolated_data101(self, table, row_checks, col_checks):
        """检查孤立数据问题"""
        num_rows = len(table)
        num_cols = len(table[0]) if table else 0
    ...
    def _check_data_type_inconsistency101(self, table, row_checks, col_checks, col_features):
        """检查数据类型不一致"""
        num_rows = len(table)
        num_cols = len(table[0]) if table else 0
    ...
    def _check_null_patterns101(self, table, row_checks, col_checks):
        """检查空值模式异常"""
        num_rows = len(table)
        num_cols = len(table[0]) if table else 0
    ...
    def _check_format_inconsistency101(self, table, row_checks, col_checks):
        """检查格式不规范"""
        num_rows = len(table)
        num_cols = len(table[0]) if table else 0
    ...
    def _print_validation_results101(self, row_checks, col_checks):
        """打印验证结果"""
    ...
    # 工具函数
    def _looks_like_percentage101(self, value):
        """判断是否看起来像百分比"""
        if not value:
            return False
    ...
    def _check_cell_validity101(self, cell, row_idx, col_idx):
        """
        检查单元格数据的有效性
        """
    ...
    def _generate_row_feature101(self, score):
        """
        根据行数据质量生成特征标记
        """
        if score >= 0.9:
    ...
    def _generate_col_feature101(self, score):
        """
        根据列数据质量生成特征标记
        """
        if score >= 0.9:
    ...
    def _generate_overall_feature101(self, score):
        """
        生成整体表格的特征标记
        """
        if score >= 0.9:
    ...
    def _contains_digits101(self, cell):
        """
        判断是否包含数字
        """
        if cell is None:
    ...
    def step7_validate_and_mark_preserve_headers101(self, final_table):
        """
    ...
    def step9_save_to_excel_with_names101(self, tables_data, table_names, output_file):
        """
        将多个表格保存到Excel，使用指定的表格名称作为Sheet名称
        tables_data: 列表，每个元素是一个表格的完整数据
        table_names: 列表，每个元素是对应表格的名称
        """
        import openpyxl
        from openpyxl.styles import Alignment, Font, Border, Side
        from openpyxl.utils import get_column_letter

        print(f"\n=== 保存到Excel（使用自定义Sheet名称）===")
        print(f"要保存{len(tables_data)}个表格到: {output_file}")
        ...
        # 处理每个表格
        for table_idx, (table, table_name) in enumerate(zip(tables_data, table_names)):
            if not table:
                print(f"表格{table_idx}为空，跳过")
                continue
            ...
            if not table_name or table_name.strip() == "":
                sheet_name = f"表格{table_idx + 1}"
            else:
                sheet_name = self._clean_sheet_name(table_name)

            print(f"  表格{table_idx + 1}: '{table_name}' -> Sheet: '{sheet_name}'")
            ...
            print(f"    尺寸: {num_rows}行 × {num_cols}列")
