...
>
> **三层能力**：全栈工程（Vue 3 + Flask + 150+ 模块）→ Agent 框架（自研零依赖 harness，含 ReAct 推理 + HITL 兜底）→ 行业应用（12 家银行 2020-2024 真实数据）
>
...
> 完整覆盖「PDF 表格解析 → 数据审核 → 会计勾稽 → 可视化看板 → RAG 智能问答」全链路：

![完整流程演示](docs/screenshots/金融PDF解析及RAG检索流程.gif)
